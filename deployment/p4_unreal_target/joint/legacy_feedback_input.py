"""Read legacy vehicle telemetry and publish P4 odometry in the selected ROS domain.
...
from ue_navigation import UENavigation

class Adapter:
 def __init__(self):
  self.contexts=[];self.nodes=[];self.executors=[];self.running=True
  for domain in (int(os.environ['P4_LEGACY_DOMAIN']),int(os.environ['ROS_DOMAIN_ID'])):
  ...
  self.ue_navigation=UENavigation(self)
 def p3_pose(self,m):
