...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("topics", "map_local", "level_semantics"), "selected_configured_global", "config error: topics.map_local.level_semantics must be 'l0_platform_window'"),
        (("map_pyramid", "base_resolution_m"), 0.25, "config error: map_pyramid.base_resolution_m must be 0.2"),
        (("map_pyramid", "resolution_scale_factors"), [1, 2, 4, 8, 16], "config error: map_pyramid.resolution_scale_factors must be [1, 2, 4, 8, 16, 20]"),
        (("map_pyramid", "maximum_level"), 4, "config error: map_pyramid.maximum_level must be 5"),
        ...
        (("map_pyramid", "target_axis_cells"), 257, "config error: map_pyramid.target_axis_cells must be 256"),
        ...
        (("static_inputs", "platform_profile", "schema"), "wrong/v1", "config error: static_inputs.platform_profile.schema must be 'lunar-platform-profile/v1'"),
        (("static_inputs", "platform_profile", "runtime_path"), "/tmp/profile.yaml", "config error: static_inputs.platform_profile.runtime_path must be '/etc/lunar_navigation/platform_profile.yaml'"),
    ],
)
