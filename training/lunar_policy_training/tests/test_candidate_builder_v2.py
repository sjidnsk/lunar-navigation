...
def test_long_single_contour_uses_farthest_fill_after_its_representative() -> None:
    ...
    batch = CandidateBuilderV2(SensorGeometry(80.0, 2.0 * math.pi)).build(
...
def test_candidate_result_does_not_depend_on_argwhere_traversal(monkeypatch: pytest.MonkeyPatch) -> None:
    world = _world_with_frontier()
    baseline = CandidateBuilderV2().build(world, _mission(), Pose2(450.0, 512.0), _projection())
    ...
    perturbed = CandidateBuilderV2().build(world, _mission(), Pose2(450.0, 512.0), _projection())
