...
def _wheel_capability() -> bridge_api.WheeledCapability:
    ...
    capability.minimum_body_z_m = 0.0
    capability.maximum_body_z_m = 1.29996
    capability.maximum_forward_speed_mps = 1.5
    capability.maximum_reverse_speed_mps = 1.5
    ...
    capability.maximum_slope_rad = 0.3490658503988659
    capability.minimum_clearance_m = 0.2
    yaw_bin_step = 2.0 * math.pi / 64.0
    arc_yaw = math.pi / 16.0
    arc_x = math.sin(arc_yaw)
    arc_y = 1.0 - math.cos(arc_yaw)
    primitives = []
    for primitive_id, kind, x, y, yaw in (
        ("forward", bridge_api.WheelPrimitiveKind.FORWARD, 0.2, 0.0, 0.0),
        ("reverse", bridge_api.WheelPrimitiveKind.REVERSE, -0.2, 0.0, 0.0),
        (
            "forward-arc",
        ...
        primitive.kind = kind
        primitive.relative_end_pose = _pose(x, y, 0.0, yaw=yaw)
        primitives.append(primitive)
    stop = bridge_api.WheelMotionPrimitive()
    stop.primitive_id = "stop-switch"
...
def _legged_capability() -> bridge_api.LeggedCapability:
    ...
    capability.maximum_linear_acceleration_mps2 = 1.0
    ...
    for primitive_id, kind, x, y in (
        ...
        primitive.primitive_id = primitive_id
        primitive.kind = kind
        primitive.body_frame_displacement_m = _vec3(0.2 * x, 0.2 * y, 0.0)
        primitives.append(primitive)
    yaw_bin_step = 2.0 * math.pi / 64.0
...
class _ProxyEpisode:
    ...
    def build_request(
        ...
        request.mission_id = f"proxy-mission-{self.scenario.scenario_seed}"
        request.mission_revision = 1
        request.platform_id = f"proxy-{self.platform_type.lower()}"
        request.capability_version = f"{self.platform_type.lower()}-capability-v2"
        request.global_map_generation = self.step + 1
        request.local_map_generation = self.step + 1
        ...
        if self.platform_type == "WHEELED":
            ...
            state.body_pose = _pose(*self.position, 0.33)
            request.current_state = state
        ...
        request.config.global_map.base_resolution_m = (
            request.world.global_map.resolution_m
        )
