"""Unified episode metrics and fixed-seed episode bootstrap confidence intervals."""
...
import numpy as np
from .baseline_core import ALL_METHODS
EPISODE_FIELDS: Final = ('method', 'scale_profile', 'scenario_key', 'scenario_seed', 'terrain_seed', 'start_pose_seed', 'evaluation_seed', 'success', 'final_coverage', 'steps_to_99_success_only', 'path_length_to_99_success_only', 'coverage_auc_over_steps', 'coverage_per_meter', 'invalid_action_count', 'planner_failure_count', 'safety_violation_count', 'termination_reason', 'coverage_curve')
BOOTSTRAP_METRICS: Final = ('success_rate_under_fixed_step_budget', 'mean_final_coverage', 'steps_to_99_success_only', 'path_length_to_99_success_only', 'coverage_auc_over_steps', 'coverage_per_meter', 'invalid_action_count_mean', 'planner_failure_count_mean', 'safety_violation_count')
ZERO_DISTANCE_POLICY: Final = 'zero_when_no_travel/v1'
_TERMINATION_REASONS: Final = frozenset({'success_done', 'failure_done', 'stagnation_done', 'no_candidate_done', 'safety_done'})
...
def build_episode_result(*, method: str, scale_profile: str, scenario_key: str, scenario_seed: int, terrain_seed: int, start_pose_seed: int, evaluation_seed: int, coverage_curve: Sequence[float], cumulative_path_length_curve: Sequence[float], steps_executed: int, invalid_action_count: int, planner_failure_count: int, safety_violation_count: int, termination_reason: str, max_steps: int, success_threshold: float, zero_distance_policy: str) -> EpisodeResult:
    ...
    if not isinstance(success_threshold, (int, float)) or not math.isfinite(float(success_threshold)) or (not 0.0 < float(success_threshold) <= 1.0):
    ...
    first_success = next((index for (index, value) in enumerate(coverage) if value >= success_threshold), None)
    ...
    auc = sum(((coverage[index] + coverage[index + 1]) * 0.5 for index in range(max_steps))) / max_steps
    total_path = path[-1]
    coverage_per_meter = coverage[-1] / total_path if total_path > 0.0 else 0.0
    return EpisodeResult(method=method, scale_profile=scale_profile, scenario_key=scenario_key, scenario_seed=scenario_seed, terrain_seed=terrain_seed, start_pose_seed=start_pose_seed, evaluation_seed=evaluation_seed, success=success, final_coverage=coverage[-1], steps_to_99_success_only=first_success if success else None, path_length_to_99_success_only=path[first_success] if success else None, coverage_auc_over_steps=float(auc), coverage_per_meter=float(coverage_per_meter), invalid_action_count=invalid_action_count, planner_failure_count=planner_failure_count, safety_violation_count=safety_violation_count, termination_reason=termination_reason, coverage_curve=coverage)
...
def validate_episode_result(result: EpisodeResult) -> None:
    ...
    if result.method not in ALL_METHODS:
        raise MetricError('episode method is not a supported evaluation method')
    ...
    finite_values = (result.final_coverage, result.coverage_auc_over_steps, result.coverage_per_meter)
    ...
    for name in ('invalid_action_count', 'planner_failure_count', 'safety_violation_count'):
    ...
    if result.success:
        if type(result.steps_to_99_success_only) is not int or result.steps_to_99_success_only < 0 or (not isinstance(result.path_length_to_99_success_only, (int, float))) or (not math.isfinite(float(result.path_length_to_99_success_only))) or (result.path_length_to_99_success_only < 0.0) or (result.final_coverage < 0.99):
            raise MetricError('success-only metrics are invalid for a successful episode')
    elif result.steps_to_99_success_only is not None or result.path_length_to_99_success_only is not None:
...
def episode_record(result: EpisodeResult) -> dict[str, object]:
    ...
    return {name: list(result.coverage_curve) if name == 'coverage_curve' else getattr(result, name) for name in EPISODE_FIELDS}

def bootstrap_episode_indices(episode_count: int, resample_count: int, bootstrap_seed: int) -> np.ndarray:
    ...
    return rng.integers(0, episode_count, size=(resample_count, episode_count), dtype=np.int64).astype('<i8', copy=False)
...
def summarize_episodes(episodes: Sequence[EpisodeResult], *, bootstrap_resamples: int, bootstrap_seed: int) -> tuple[dict[str, object], dict[str, object]]:
    rows = tuple(episodes)
    if not rows:
    ...
    summary: dict[str, object] = {'method': ordered[0].method, 'scale_profile': ordered[0].scale_profile, 'episode_count': len(ordered), **values}
    indices = bootstrap_episode_indices(len(ordered), bootstrap_resamples, bootstrap_seed)
    ...
    for name in BOOTSTRAP_METRICS:
        ...
        if estimate is None:
            intervals[name] = {'estimate': None, 'ci95_low': None, 'ci95_high': None, 'valid_resample_count': 0}
            continue
        ...
        intervals[name] = {'estimate': estimate, 'ci95_low': float(low), 'ci95_high': float(high), 'valid_resample_count': int(metric_samples.size)}
    audit = {'schema_version': 'fixed_seed_episode_bootstrap_95ci/v1', 'method': ordered[0].method, 'scale_profile': ordered[0].scale_profile, 'sampling_unit': 'episode', 'rng_algorithm': 'numpy.random.PCG64', 'bootstrap_seed': bootstrap_seed, 'resample_count': bootstrap_resamples, 'episode_count': len(ordered), 'sample_index_dtype': '<i8', 'sample_index_order': 'C', 'sample_index_shape': list(indices.shape), 'sample_indices_sha256': bootstrap_indices_sha256(indices), 'episode_order': [{'scenario_key': row.scenario_key, 'scenario_seed': row.scenario_seed, 'terrain_seed': row.terrain_seed, 'start_pose_seed': row.start_pose_seed, 'evaluation_seed': row.evaluation_seed} for row in ordered], 'percentiles': [2.5, 97.5], 'metrics': intervals}
    return (summary, audit)

def _episode_order_key(result: EpisodeResult) -> tuple[object, ...]:
    return (result.scenario_key, result.scenario_seed, result.terrain_seed, result.start_pose_seed, result.evaluation_seed)
...
def _summary_values(episodes: Sequence[EpisodeResult]) -> dict[str, object]:
    ...
    return {'success_rate_under_fixed_step_budget': float(np.mean([float(row.success) for row in episodes], dtype=np.float64)), 'mean_final_coverage': _finite_mean((row.final_coverage for row in episodes)), 'steps_to_99_success_only': _finite_mean((float(row.steps_to_99_success_only) for row in success_rows)) if success_rows else None, 'path_length_to_99_success_only': _finite_mean((float(row.path_length_to_99_success_only) for row in success_rows)) if success_rows else None, 'coverage_auc_over_steps': _finite_mean((row.coverage_auc_over_steps for row in episodes)), 'coverage_per_meter': _finite_mean((row.coverage_per_meter for row in episodes)), 'invalid_action_count_mean': _finite_mean((float(row.invalid_action_count) for row in episodes)), 'planner_failure_count_mean': _finite_mean((float(row.planner_failure_count) for row in episodes)), 'safety_violation_count': sum((row.safety_violation_count for row in episodes))}
