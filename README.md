...
启用后，一条月表 Action 先计算一次按轮式包络膨胀的全局路线；随后以 8 m 路线前瞻重复产生严格局部参考，直到 odometry 进入最终目标容差。每段局部搜索仍限制为 1 s，整个 Action 默认限制为 300 s；取消、替换和失败都会发布空轮式参考。该模式不替代外部全局图生产者或独立轮式控制器。
