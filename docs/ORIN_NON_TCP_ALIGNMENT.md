...
| 实机配置 | 盲区余量 3.0 m、能力速度 0.2 m/s、容差 0.1 m / 0.05 rad | 保留实机 P3 输入、传感器配置和物理通过性限制 |

`wheel_orin.yaml` 的盲区余量增加在车体外接圆半径上，总半径约 3.719 m，
不是把半径 3 m 范围全部标成可通行。已有障碍、真实出口与支撑平面认证仍有效。
...
当时切换的本地与远端平台配置 SHA256 一致，盲区余量为 2.0 m。后续用户将源码配置调整为
3.0 m；本次整合没有重新部署到 Orin，因此现场运行值仍需重新核对。
...
## 2026-09-18 地形与地图边界整合

`fix/orin-terrain-sync` 的 5×5 稳健轮式地形判定、完整未知地图边界、细图重新派生和回归测试已整合到本分支。合并保留本分支已有的目标位置/朝向容差参数；盲区余量按用户后续要求统一为 3.0 m。

本机先导入测试并确认旧实现出现 3 个核心测试失败，3 m 配置出现 2 个预期失败；应用生产改动后核心 CTest 17/17、配置测试 4/4 通过。最终整包验证结果以本次提交时重新执行的 Jazzy 构建与测试为准。Humble/Orin 未在本轮重新部署或运行，因此前述原生失败和现场 NOT_RUN 边界仍然有效。
