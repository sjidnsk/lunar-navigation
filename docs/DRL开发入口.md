...
| 决策图 | `task_graph_v3`：2 m 合法路径覆盖、8 m 局部连接、1.2 路程伸长稀疏化；保留必要补点、回退和任务外绕行。 |
...
净空优先选点的原型、C=5 敏感性试验等历史探针不因整理文档而自动成为正式算法。当前决策图仍允许必要近邻节点；这次整理没有改变算法、奖励或学习参数。
...
本次只整合和验证代码，不启动训练或配对实验。下一次配对比较应在同一提交、同一导航版本下运行两组，使用新实验目录，例如：

```bash
scripts/drl/compare_actor.sh --output-dir training-output/actor-bounded-clearance-v1
# 仅恢复上述新目录中、相同代码和配置下中断的实验：
scripts/drl/compare_actor.sh --output-dir training-output/actor-bounded-clearance-v1 --resume
```
...
模型恢复是否兼容由观察、动作、Critic 和配置协议决定，不由分支名字决定。本次代码整理本身不要求清空兼容检查点；但旧 GRU/旧图协议不兼容新策略。导航改变后的公平 A/B 比较使用新实验目录，旧结果仍可用于诊断。
