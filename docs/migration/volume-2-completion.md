...
2026-08-08 修订：以下增量中的实时/累计燃料链已由固定、不可递减的单跳 Δv 包络取代；
`HopperPropellantState` 不再进入规划快照、训练或外部 RViz 桥。其余三平台几何、运动学、落区、
抛物线和飞行管能力继续有效。
...
- 飞跃平台固定单跳 Δv 包络、精确单跳落点、简单抛物线与完整飞行管认证；
- 三平台 ROS 输出不变量和 RViz 尺寸、路线、落区、弹道与 required/available Δv 证据；
