...
**Goal:** 接收课题三实时 0.2 m 局部图与左相机位姿，生成规划器可安全消费的局部图和车体状态，并提供可启动的 Task3 规划联调入口。

**Architecture:** 保留现有 `luna_t3_map_adapter` 的只读、ROI 有界全局 SQLite 适配职责，在同一包中新增局部证据转换和局部 ROS 节点；新增独立 `luna_t3_pose_adapter` 将相机位姿变换为车体里程计和 TF。部署运行时通过明确的 `input_mode: external_canonical | task3_adapted` 选择是否启动这两个输入适配器；`PlanMotion` 仍是唯一的高层输出，控制器契约不明时不实现或猜测底层运动命令。

**Tech Stack:** ROS 2 Humble、Python 3、rclpy、tf2_ros、grid_map_msgs、nav_msgs、NumPy、SQLite、ament_cmake_python、pytest、ament_cmake_pytest。
...
- 局部输入和输出均严格为 `0.2 m`；不得插值、重采样或改变格数。
...
| `ros2_ws/src/luna_t3_map_adapter/python/luna_t3_map_adapter/local_map_adapter_node.py` | Task3 局部图订阅、TF 查询、tile evidence 读取和 canonical 局部图发布。 |
| `ros2_ws/src/luna_t3_map_adapter/config/task3_local_map_adapter.yaml` | 局部地图 topic、frame、安全映射、缓存和新鲜度配置。 |
| `ros2_ws/src/luna_t3_map_adapter/launch/task3_live_integration.launch.py` | 以 Task3 适配模式共同启动全局、局部、位姿适配与 planner。 |
| `ros2_ws/src/luna_t3_pose_adapter/...` | 相机到车体刚体变换、标定加载、状态分类和 ROS 发布节点。 |
| `deployment/luna_runtime/{config.py,process.py,build.py}` | 输入模式校验、启动命令选择及新包构建选择。 |
| `deployment/config/{runtime.default.yaml,task3-adapted.runtime.yaml,task3-adapters.default.yaml}` | generic 与 Task3 输入模式配置；Task3 SQLite、topic、frame 与标定路径均可由操作者覆盖。 |
...
**Interfaces:**
- Consumes: `/Car/T3/mapping/grid_map`, `tf2_ros.Buffer.lookup_transform("odom", "map", source_stamp)` and a read-only `TileEvidenceProvider`.
- Produces: `/environment/map_local` with the ten required layers, `frame_id="odom"`, source timestamp, reliable/transient-local depth-1 QoS; diagnostics reasons `LOCAL_MAP_RESOLUTION_INVALID`, `LOCAL_MAP_FRAME_INVALID`, `LOCAL_MAP_TF_UNAVAILABLE`, `LOCAL_MAP_EVIDENCE_INCOMPLETE`.

- [ ] **Step 1: Write failing node tests with fakes**

```python
def test_node_publishes_unchanged_02m_dimensions_in_odom_frame():
    node = LocalMapAdapterNode(transformer=fake_planar_transform(), evidence_provider=complete_provider())
    node.on_source_grid(task3_grid_map(resolution=0.2, width=300, height=300))
...
def test_node_does_not_publish_when_tf_or_tile_evidence_is_unavailable():
    node = LocalMapAdapterNode(transformer=missing_transform(), evidence_provider=missing_provider())
    node.on_source_grid(task3_grid_map(resolution=0.2))
    assert node.publisher.messages == []
    assert node.last_reason in {"LOCAL_MAP_TF_UNAVAILABLE", "LOCAL_MAP_EVIDENCE_INCOMPLETE"}
```
...
class LocalMapAdapterNode(Node):
    def _on_source_grid(self, message: GridMap) -> None:
        ...
        transform = self._transformer.odom_from_map(message.header.stamp)
...
Use only a planar `odom_from_map` transform. Add `tf2_ros`, `geometry_msgs`, `diagnostic_msgs` and required ROS message dependencies to `package.xml`; install the second script in CMake. The YAML must declare source/output topic names, frames, `0.2`, occupancy threshold, semantic IDs, cache size and `max_age_s`.
...
**Files:**
- Create: `ros2_ws/src/luna_t3_pose_adapter/CMakeLists.txt`
- Create: `ros2_ws/src/luna_t3_pose_adapter/package.xml`
- Create: `ros2_ws/src/luna_t3_pose_adapter/python/luna_t3_pose_adapter/{__init__.py,pose_conversion.py,pose_adapter_node.py}`
- Create: `ros2_ws/src/luna_t3_pose_adapter/scripts/luna_t3_pose_adapter_node.py`
- Create: `ros2_ws/src/luna_t3_pose_adapter/config/task3_pose_adapter.yaml`
- Test: `ros2_ws/src/luna_t3_pose_adapter/test/test_pose_conversion.py`
- Test: `ros2_ws/src/luna_t3_pose_adapter/test/test_pose_adapter_node.py`

**Interfaces:**
- Consumes: `/Car/T3/semantic/current_pose` (`map -> car_stereo_left_optical_frame`) and `camera_to_base.yaml` containing a rigid transform with source metadata.
- Produces: `/localization/odometry` (`odom -> base_link`), `/localization/status`, static `map -> odom`, dynamic `odom -> base_link`.

- [ ] **Step 1: Write failing conversion tests**

```python
def test_camera_pose_becomes_base_pose_using_calibrated_rigid_transform():
    output = transform_camera_odometry(camera_pose_at(10.0, 2.0), calibration(offset_x=0.4))
    assert output.child_frame_id == "base_link"
    assert output.pose.pose.position.x == pytest.approx(10.4)
    assert output.header.frame_id == "odom"

def test_invalid_calibration_or_nonfinite_covariance_produces_invalid_status():
    assert classify_localization(bad_odometry(), valid_calibration()) == "INVALID"
    with pytest.raises(CalibrationError, match="CALIBRATION_FRAME_INVALID"):
        load_calibration(invalid_calibration_path())
```

- [ ] **Step 2: Run the tests to verify RED**

Run: `PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -q ros2_ws/src/luna_t3_pose_adapter/test/test_pose_conversion.py ros2_ws/src/luna_t3_pose_adapter/test/test_pose_adapter_node.py`

Expected: FAIL because `luna_t3_pose_adapter` does not exist.

- [ ] **Step 3: Implement pure transform and ROS publication**

```python
def transform_camera_odometry(source: Odometry, calibration: CameraToBase,
                              odom_from_map: RigidTransform) -> Odometry:
    # Compose T_odom_base = T_odom_map * T_map_camera * T_camera_base;
    # transform velocity and 6x6 covariance with the rigid-body adjoint.

class PoseAdapterNode(Node):
    # Publish map->odom once in map_locked mode, broadcast dynamic odom->base_link,
    # and publish VALID/DEGRADED/INVALID without renaming an untransformed camera pose.
```

`camera_to_base.yaml` must reject wrong source/target frame names, non-unit quaternion, non-finite translation and missing provenance. `map_locked` is the only initial mode; an external odom transform source is a later additive mode.

- [ ] **Step 4: Run package tests and build**

Run: `source /opt/ros/humble/setup.bash && colcon build --packages-select luna_t3_pose_adapter --cmake-args -DCMAKE_BUILD_TYPE=RelWithDebInfo && source install/setup.bash && colcon test --packages-select luna_t3_pose_adapter --event-handlers console_direct+`

Expected: conversion, status and publisher tests pass; package exports no robot-control command publisher.

- [ ] **Step 5: Commit**

```bash
git add ros2_ws/src/luna_t3_pose_adapter
git commit -m "feat(task3): adapt camera pose to vehicle localization"
```
...
- Produces: `external_canonical` launches only planner; `task3_adapted` launches global/local/pose adapters plus planner and passes the selected adapter YAML paths.
...
def test_task3_mode_launches_single_composed_launch_and_builds_both_adapters():
    ...
    assert "luna_t3_pose_adapter" in plan.packages
...
**Interfaces:**
- Consumes: active task、canonical Task3-adapted maps、transformed odometry、`PlanMotion` POINT goal、`MotionExecutionFeedback`。
...
Add a Task3-adapted fixture that publishes canonical maps and transformed `odom -> base_link`; do not add a controller command publisher. Implement the client surface:
...
- Spec coverage: Tasks 1–3 implement unresampled ten-layer local evidence; Task 4 implements camera-to-body localization; Task 5 implements runtime selection and package delivery; Task 6 fixes the Action/feedback integration boundary and documentation; Task 7 defines target-host proof. No spec requirement is omitted.
...
- Type consistency: all local map tasks use `CanonicalLocalGrid`, `LocalMapPolicy`, `ObservationLedger`, `GridMap`; all pose tasks use `CameraToBase`, `Odometry`, `LocalizationStatus`; deployment mode is consistently `external_canonical | task3_adapted`.
