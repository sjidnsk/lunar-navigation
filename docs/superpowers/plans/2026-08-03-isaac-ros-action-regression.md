...
- Test: external `ros2_ws/src/lunar_isaac_validation/test/{test_trajectory_checks.py,test_action_assertions.py,test_regression_runner.py,test_reports.py}`
...
Apply the same literal-only correction to the regression-runner test's `_task8_wheel_case()` and `_task8_wheel_bundle()` fixture: preserve its global grid origin, centre only local grids at `(-5.0625,-5.0625)`, and lock wheel cell `[40,40]`, projected position `[0.0,0.0,0.0]`, and maximum error `0.0625`. No other runner fixture or orchestration behavior may change.
