...
候选位置继续从 4 m observed-only 全局图生成；候选潜在增益则从机器人已经持有的稀疏
0.2 m 已观测状态批量估算，未知单元的障碍值不得从 truth 补入。精细增益按物理面积换算后写入
既有候选特征，不能修改观测状态或 reward。机器人当前所在的 4 m 全局栅格必须在估算前排除；
原地不动不构成探索候选，也不能触发平台规划。
