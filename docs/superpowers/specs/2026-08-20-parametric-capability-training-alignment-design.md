...
10 m 是传感器有效观测距离，不是地面候选动作距离、全局路径范围或滚动局部规划范围。地面候选与全局路线没有固定的 30 m 总距离上限：当前 C++ ground global cost tree 会搜索当前全局安全地图内的完整连通区域。Python 仍传入的 `maximum_edge_distance_m=30.0` 是兼容旧桥接口的证据字段，ground cost-tree 实现不使用它裁剪搜索；不得把它描述成地面“30 m 搜索域”。滚动局部规划仍按既有 4 m（WHEELED）/3 m（LEGGED）前视连续推进，它们也不是宏动作总路程上限。本次不调整这些规划算法或前视配置。
