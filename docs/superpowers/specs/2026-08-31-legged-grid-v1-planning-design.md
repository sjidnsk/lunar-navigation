...
最大坡度和 `3 x 3` 邻域台阶规则一致。`body_center_feasible` 是仅用于二维连通性和
搜索引导的必要条件：以 `hard_feasible == 0 || step_feasible == 0` 为危险源，并按
扩展机身矩形的内接圆半径生成中心可行掩码。内接圆半径固定为
`min(body_extent.x, body_extent.y) / 2 + minimum_body_clearance`；当前生产配置为
`0.33 / 2 + 0.30 = 0.465 m`。地图边界按相同半径处理。该掩码只能提前排除任何
机身朝向都不可能通过的位置，不能替代带朝向矩形边认证。

台阶邻域在投影构建时对每格只计算一次，不得在搜索边验证中重复计算。
`roughness_m` 继续由 elevation 内部派生，仅保留为现有地形代价；它不是新增输入层，
也不成为新的硬准入条件。
...
普通区域和狭窄区域统一使用 `32` 个相对航向格；狭窄区域只保留现有半 XY 分辨率，
不再增加航向格数量。为了让一个旋转动作精确对应一个航向格，冻结能力配置中的
左右旋转原语同步从 `±pi/32` 调整为 `±pi/16`。`motion_mode` 继续参与状态键，因为
模式切换参与边代价。本阶段保持现有单键状态逻辑，不增加多标签、优势裁剪或状态
签名，也不增加动作原语种类。
...
目标格为源点，在 `LeggedTraversalProjection.body_center_feasible` 掩码上执行现有
八邻域传播并禁止对角穿角，记录：
...
起点距离为无穷时，说明起点与全部目标在机身中心必要可行掩码上断开，直接返回
`LEGGED_NO_PATH`。距离有限时，距离场进入足式 ARA* 的主启发函数：按当前状态所在
格的距离、对应目标容差和足式 `cost_scales` 换算为非负搜索代价，并与欧氏目标区域
代价取较强者。`guidance` 继续保留为稳定次级排序。距离场只改变搜索顺序，不替代
足式动作边认证，也不得把二维连通性当作足式路径成功；首解仍必须由现有动作边
逐边认证。
...
首版不预计算 `(yaw_bin, primitive)` 完整扫掠掩码。搜索状态统一为 `32` 航向格，
并以 `±pi/16` 旋转原语保持状态格闭合。只有本设计的生产配置基准仍不能达到性能
验收时，才单独设计动作原语扫掠掩码；不得在首版中增加多标签状态、复杂优势裁剪
或新的动作原语集合。
...
新增生产局部性能场景：`64 m` 局部地图、`0.2 m` 分辨率、`4 m` 局部目标、生产
...
足式 `grid_traversability_v1` 接入现有月面滚动调度，同一 Action 复用一次全局路线。
初版按 `3.0 m` 前瞻发布足式局部分段，并等待当前分段终点到达后再规划下一段；
该历史行为已由文末的 `4.0 m` 前瞻和 `1.5 m` 提前重规划修订替代。每个新周期使用
当次冻结的可通行性快照，轮式滚动触发规则保持不变。
...
以足式机身中心必要可行掩码构建多源距离场，状态键改为请求起点与起始航向的
...
## 2026-08-31 机身中心距离场与 32 航向格实施结果

足式投影已增加 `body_center_feasible`：危险源仍只来自既有占据、高程坡度和台阶
结论，按扩展机身内接圆半径以及地图物理边界生成必要可行掩码。该掩码用于多目标
距离场和搜索前二维连通性判断；每条实际动作边仍由原有带朝向矩形 `SweepBody`
认证。距离变换直接返回中心到危险栅格面积的距离，因此实现不额外增加半格膨胀，
避免把仍可能通过的位置提前判死。

足式 ARA* 主启发函数现使用该距离场，并按现有足式代价尺度换算；欧氏目标区域
下界和稳定 guidance 保留。固定长障碍回归场景在旧欧氏主启发下展开 `4199` 个状态，
改动后展开 `62` 个状态；机身不可能通过的走廊从进入搜索并展开 `1` 个状态改为在
搜索前返回 `LEGGED_NO_PATH`、展开 `0` 个状态。

普通和狭窄区域的足式状态键均统一为 `32` 个航向格，狭窄区域仍只使用半 XY
分辨率。生产左右旋转原语同步为 `±pi/16`，能力版本更新为
`quad48-approved-baseline-v2`；两次旋转到达 `pi/8` 的行为测试通过。轮式与 Hopper
源码、配置和外部接口未修改。

本机 Jazzy 最终重新构建 `lunar_planning_msgs`、`lunar_pure_planner_core` 和
`lunar_pure_planner_ros` 成功；core CTest `22/22`、ROS CTest `18/18`、指定隔离与
launch 契约测试 `35/35` 通过。core 首轮中未改动的轮式 `750 m` 性能用例在
`3004.21 ms` 处触发一次 `3 s` deadline；该单场景隔离复跑成功且最慢周期为
`2267.05 ms`，随后完整 core 复跑 `22/22` 通过。本次未运行新的 RViz 实验；
`Humble`、`Orin`、`DDS`、`rosbag` 和实车均为 `NOT_RUN`。

## 2026-08-31 足式滚动终端与提前重规划修订

足式滚动前瞻统一为 `4.0 m`，提前重规划步长固定为 `1.5 m`。每次成功发布分段时
记录当时的实际路线投影进度；后续实际进度增加达到 `1.5 m`，或当前门户已经到达时，
触发下一次局部规划。新周期的前瞻从当前实际路线投影进度重新计算，不得从上一门户
进度继续累加。这样正常情况下保留约 `2.5 m` 已规划尾段。局部图刷新本身仍不抢占
足式分段，轮式条件分支不变。

中间门户终端不能只满足目标容差和入边认证，还必须存在至少一个完整长度、无旋转、
相对本段起点至门户方向具有正投影的平移动作，并通过现有 `SweepBody`。该动作仅作为
终端可继续向前移动的见证，不追加到输出轨迹；没有合格后继时拒绝该终端并继续搜索
其他状态或门户。原路后退不构成正向后继，精确最终目标不执行该条件。实现复用现有
动作原语和边认证，不增加地图层、路线走廊、并行搜索或发布前安全流程。

足式中间门户在排序和最多 `32` 个候选截断之前，必须使用与局部距离场相同的
`body_center_feasible` 过滤。点单元虽有有限高程且未占据、但不满足坡度、台阶或扩展
机身中心净空的候选不得占用候选名额；生成器继续检查后续横向和回退位置。该过滤只
改变足式中间门户准入，轮式门户和带任务容差的精确最终目标保持原行为。实际动作边仍
由 `SweepBody` 认证，门户过滤不得替代搜索或轨迹认证。
