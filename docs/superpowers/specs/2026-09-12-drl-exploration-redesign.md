...
创建：2026-09-12；统一修订：2026-09-15，含 30 倍仿真、并行/学习参数及实施前复审。状态：已获实施授权，按[实施清单](../plans/2026-09-15-drl-exploration-redesign.md)执行；具体完成和运行证据另行记录。
...
栅格视域使用确定的射线/格角约定，保证同一栅格的可见判定不因参考计算与逐步模拟采用不同采样规则而矛盾。原生 FREE 分类需要 3×3 高程邻域；若只回传射线命中中心的高程，首个 BLOCKED 格可能挡住产生其分类所需的邻居，造成封闭墙边永久 UNKNOWN。因此训练传感器明确采用**有效中心地形测量近似**：先用原生测量算子取得中心的坡度、起伏和正向高差，再按同一二维视域裁剪，只发布命中中心的这些测量及中心高程。隐藏邻居的高程、分类和面积不发布、不计覆盖。
...
2. 先求可能观察任务内未知格的站位关系：候选站位不能是导航状态 `M_t` 已知阻塞，距离不超过量程，视线不能穿过固有地形 `B_t` 的已知阻塞。未知保持潜在可能，不能先假定自由而奖励成片面积。
...
当前位置使用实际平台位姿的临时锚点，不能用附近固定格中心替代；选择当前位置的八个方向只要求原地转向。稳定采样节点、临时锚点与实际观测历史分别保存，不因图重建丢掉世界坐标下的访问信息。
