...
创建：2026-09-12；统一修订：2026-09-15。状态：设计交付，尚未实施本版算法、训练或部署。
...
独立工作树为 `lunar-runtime/.worktrees/drl-exploration-redesign`，分支从 `integration/pure-planner-orin@5c23c13cd0605955412ba6f470ee051d4fe8a9a2` 创建。旧训练所在 `observability-coverage` 工作树仅作实现经验和失败案例来源，不直接搬入其模型、回放或全部导航改动。本次只修改设计文档，不启停训练。
...
| 仿真 | 复用现有导航、规划、跟踪闭环；8 个训练环境，20 倍仿真调度目标；暂不使用 Isaac Sim/Lab |
...
观测沿实际运动和旋转持续发生，不仅在目标终点更新。采用固定仿真时间步，20 倍只改变墙钟调度，不能把物理速度改成 4 m/s，或跳过避障、碰撞及观测中间过程。0.05 仿真秒积分、2 Hz 仿真观测沿用为工程初值，实际倍率单独测量。

栅格视域使用确定的射线/格角约定，保证同一栅格的可见判定不因参考计算与逐步模拟采用不同采样规则而矛盾。原生地形分类若需要有限邻域高程支撑，`V(q)` 必须表示这套观测与有效分类过程最终可确认的格，而不是未经适配的原始射线命中数；参考计算与环境更新共用该定义。不能在计算分母时假设能确认一个格，实际地图链路却永远不能确认它。
...
实现时先求真值可达关系，再对任务相关位置累积位图。超出任务量程邻域的位姿可从视域计算中排除，但可达关系仍允许经远处绕行；不得先裁掉任务外道路。使用共享静态场景、紧凑位图和一致的原生可见性算子，避免逐位姿调用 ROS 或复制整幅高程。1 km 初始化成本必须实测，本文不声称已经解决性能瓶颈。
...
2. 从任务内剩余未知区反向分析潜在连接，经未知区及尚未连接 `R_t` 的已知自由岛追踪可能的入口；已知阻塞证据终止相应连接。
3. 分析抵达 `R_t` 即记录入口并停止穿越 `R_t` 继续扩散。这样，无关的外部未知分支不会仅因共用一条已经走通的道路而变成任务义务。
4. 对入口保留从可达站位、在量程与遮挡条件下可以获得的新观测机会。无需站到前沿格本身；对于不能驶入但仍可看入的区域，运动连通与视线判断分别处理。
5. 得到全图任务相关前沿/观测接口集合 `F_task`；只有该集合耗尽才正常结束。

上述是算法契约。具体增量搜索必须保持运动边界与观测边界的区别，特别不能把足迹膨胀当作射线终点。前沿 utility 只计已知地图支持的边界机会，不估计成片未知高程的可见面积。针对有限分辨率的边界与分类支撑，需要第 16 节的实现测试。
...
地图存储边界仍是未知，不伪装成墙。场景生成器的物理世界范围和任务范围是两个概念；无界外部且潜在入口一直不能排除时，不能保证有限时间一定证明耗尽，也不能增加一个隐藏的任务外距离截止线来伪造完成。
...
在当前节点的少量相邻节点中选择下一跳，保留当前位置用于原地转向。候选位置总数至多 20，包含当前位置；每个位置对应 8 个世界坐标系朝向，间隔 45°，所以每次最多 160 个联合动作。
...
`ΔA` 是整个动作期间任务内首次有效观测面积；`ΔL` 是实际行驶总距离；`ΔΘ` 是累计绝对转角，而不是首尾朝向之差。任务外路程和原地旋转均计入。真实耗尽没有额外完成奖励，达到 80% 或 99% 也不另加奖励。

100 m²、10 m、π rad 是固定物理归一化单位，不使用任务面积、当前地图大小、可覆盖分母或当前扇形面积归一化。同样发现 1 m²，在小图、大图或不同验证传感器下均得到 0.01。

10 m 路程代价为 0.02，90° 转向代价为 0.0025，每次决策代价为 0.001。一次新增 50 m²、行驶 10 m、累计转向 90°，奖励为 `0.4765`；没有新增面积时为 `-0.0235`。这组权重是确认采用的训练基线，尚未被实验验证为最优。

固定单位避免大地图把单位信息收益压到极小，但不能消除拓扑难度或大图样本占比差异。课程采样和按场景分组评估分别处理这些问题，不加入随覆盖阶段改变的权重。

### 9.2 折扣与熵强度

采用回合式 `γ=1`，避免大量局部下一跳把后期覆盖收益指数衰减。它要求正确区分真实终止与训练截断；不再具有 `γ<1` 的收缩保证，需实测 Q 尺度、尾部循环及不同随机种子的稳定性。

\[
H_{\mathrm{target}}=0.01\log N_{\mathrm{valid}},\quad
...
训练调度采用有界待更新额度与背压：新增有效转移产生更新需求，Learner 批量消费；达到配置的积压上限后暂缓派发新动作，已执行动作正常收尾，所得经验不丢弃。不能把超出额度悄悄清零，再声称更新已追上采集。记录采集/s、更新/s、待更新量、实际 batch 及 Actor 版本滞后。
...
batch、学习率、回放容量和权重发布频率是工程参数，不能仅凭 GPU 型号决定其最优值。实施时以非循环图批处理作首次性能测量，再给出实际配置；这不要求先完成传统探索对照。策略数学契约和第 9 节参数不因吞吐优化改变。
...
3. 足迹膨胀、可见但不可驶入的窄处、格角和瓦片边界：参考与实际观测一致，站位与遮挡语义不混用。
...
性能分项记录真值初始化、地图导出、任务分析、图更新、Actor 推理、导航执行、SGD、保存及内存峰值。20 倍为目标，8 环境在本机是否达到该倍率需要测量。`γ=1`、低温度、固定成本及较长局部决策链可能带来价值估计与学习速度问题，不能承诺固定转移数收敛。

本次证据仅为接口阅读、文献核对、设计与文档检查。新算法实现、构建/包测试、新训练、Jazzy 闭环、Humble、Orin/DDS、真实课题三与车辆均为 `NOT_RUN`。二维遮挡是任务近似，仿真耗尽和高覆盖不自动证明真实地形可见性或现场验收通过。
...
| 奖励/折扣 | 传感器面积归一化、完成奖励、γ=0.999 | 固定物理单位、无完成奖金、γ=1、小温度与决策成本 |
...
| 运行保留 | 原生导航、8 环境、30 分钟保存 | 保留；仿真调度目标 20 倍、物理限速仍 0.2 m/s |
