...
日期：2026-09-12；2026-09-13 补充奖励尺度审查及 20 倍仿真要求。状态：本分支的设计交付，算法、接口扩展、训练和部署均未实施。
...
| 8 个环境、20 倍仿真 | 主训练继续执行原导航、规划、跟踪和碰撞闭环 |
...
上一版提出的候选奖励如下。2026-09-13 审查确认，其中路程和转角权重提高五倍缺少实验依据，并存在长通道反例；这组数值尚不能作为定版权重：
...
此候选下，实际新增 78.54 m² 为 +1，走 10 m 为 -0.05，转一圈为 -0.005。审查建议先以路程 `0.01`、转角 `0.001` 为实验基线，其余项保留，和上述候选在同种子的长通道、零增益回程及不同尺度任务中比较；这不是声明旧数值已适合新的分类覆盖任务。本次只评估方案，没有修改旧训练配置。公开分项日志，权重调整必须重新记录实验版本。

这些项已经按观测量程归一化：面积除以 `A0`，路程除以 `R`，转角除以 `2π`。相同传感器下，相同新增面积和行驶距离在大小地图上保持相同的局部收益权衡；本版建议保留这种物理尺度，不单独把面积项改成 `ΔA_new/A_T`。例如任务面积从 10,000 m² 增到 1,000,000 m²，只做该替换会使相同观测相对相同路程的收益缩小 100 倍。

地图大小仍需分别处理：状态中的面积比例采用公开且固定的 `A_T`；评估使用私有参考覆盖 `C*`，按地形和尺度分别报告完成率、到 95% 路程、成功总路程及单位参考面积路程，不跨尺度只比较累计 reward。大图累计面积奖励较大本身不证明目标错误；在同一场景、确认同一集合且不折扣时，这部分累计奖励与访问顺序无关。

若后续实验确实需要按任务尺度缩放训练回报，应在回合初始化固定一个正比例 `s`，一致缩放所有奖励项，并相应使用 `α'=sα`，才能在该场景内保持原有环境收益与熵的相对尺度。只缩放面积、保留完成奖励或固定熵系数会改变优化目标，必须作为新实验定义；一致缩放也不能修复不合理的原始权重或折扣。在线变化的可覆盖面积估计不用于奖励分母。不在本版额外引入奖励归一化模块。详见[奖励审查与数值反例](../../validation/2026-09-12-drl-exploration-redesign-review.md#7-2026-09-13-奖励尺度与批注复核)。
...
主训练使用 8 个隔离 ROS 环境，复用原生导航、路径跟踪、0.2 m/s 运动和碰撞判定，按 20 倍墙钟倍率调度。CPU 采集、GPU 学习，按环境就绪异步收取完整动作转移。

20 倍表示目标仿真时间推进速度；物理速度上限仍为 0.2 m/s，积分步长仍为 0.05 仿真秒，观测仍为 2 Hz 仿真频率，不能通过放大单步位移或降低观测密度凑倍率。达到目标时，8 环境合计约产生 320 次地图观测/墙钟秒，实际倍率按运行结果记录。每 30 分钟保存仍按墙钟计时；仿真运动超时和基础设施墙钟超时各按原本的时钟语义处理。
...
主训练保留既有导航闭环、8 环境、0.2 m/s 和 30 分钟保存，目标倍率按用户要求改为 20 倍。新定义必须整体落地，不能只替换奖励字段或网络输入数量就称为本设计完成。

## 19. 文献依据及采用边界

文献事实依据原文；2026-09-13 已重新核对 ARiADNE、HEADER 的奖励章节，并补充 SAC 的奖励尺度依据，没有把论文性能当成项目验证结果。
...
- [Soft Actor-Critic](https://proceedings.mlr.press/v80/haarnoja18b/haarnoja18b.pdf)：奖励尺度与策略熵相互关联；不能仅归一化覆盖项后沿用原温度，便认为优化目标保持不变。
