...
| 完成 | 训练真值覆盖≥99%成功终止；部署按当前任务机会耗尽结束 |
...
普通前沿是已知自由区域与未知区域的接口。当前观测机会只从完整增量地图中的已知可达区域 `R_t` 查询：哪些已知可执行站位，按当前传感器模型能继续观测任务内未知区域。站位和路径可以在任务外，收益只统计任务内。

不推演未来未知站位、通道或假想入口，不要求排除所有任务外绕行可能。前沿 utility 表达当前边界机会，不预测未知高程后方的大面积收益。决策图保留相应见证站位和已知连接；不能因稀疏采样遗漏当前有效机会。实现及边界详见[当前观测机会耗尽](../../validation/2026-09-20-current-task-opportunities.md)。

### 6.2 正常结束、成功与截断

部署使用测量侧判据：已有任务观测且当前任务观测机会耗尽时结束。不使用真值参考面积，不声称在线证明99%。输入不可用、局部候选为空和连续零收益不等于耗尽。

训练和仿真评估使用环境真值：`C=A_cov/A_E >= success_coverage`（默认0.99）即任务成功，设置 `completed=true, terminated=true`，该动作额外奖励5。即使还有观测机会，也停止当前回合；不要求观测完剩余1%。Actor输入不增加真值。

覆盖未达标但测量侧机会耗尽时，`completed=false, terminated=true`，不发完成奖励。仅达到预算时为 `truncated=true, terminated=false`，保留重置前状态自举。同一步达标且达到预算，以成功终止为准。碰撞不能成为成功。初始状态已经达标时记录零决策成功，不伪造奖励转移。

参考集合仍排除不可观测区域，定义不变。覆盖比例用集合内已覆盖栅格数除以集合总栅格数，避免等面积乘法导致恰好99%向下舍入。空参考不视为成功。80%继续作为中期评估里程碑。

地图存储边界仍是未知，不伪装成墙。生成器 v5 提供完整洞室/通道、岩石与陨坑解析支撑及12 m上下文；场景物理范围和任务范围仍分开。
...
`d_t` 只表示真实终止。课程上限为采样截断，使用重置前的最后状态自举；不能用下一张地图的初始状态替代。覆盖成功或机会耗尽均为真实终止，目标直接取本步奖励；不对终止状态计算后继 soft value。
...
-0.001+b_{\mathrm{done}}\mathbf{1}[s_t=1],\qquad b_{\mathrm{done}}=5.
\]

`ΔA` 是整个动作期间任务内首次有效观测面积；`ΔL` 是实际行驶总距离；`ΔΘ` 是累计绝对转角，而不是首尾朝向之差。任务外路程和原地旋转均计入。2026-09-20按用户要求，`s_t` 表示真实可探索覆盖率达到99%的任务成功，最后一条转移额外奖励5（completion_bonus）。低于99%的耗尽、预算截断和碰撞不发完成奖励；80%没有额外奖励。结束后必须重置，初始已结束不伪造转移。
...
达到上限：结束本次采样、记录 `truncated=true`、保留最后状态自举并换图。覆盖成功或探索耗尽：`terminated=true`。连续零新增用于诊断，不再叠加一个“连续若干次无收益即完成/失败”的回合机制。前沿方法用于性能对照，不是预算标定或开始训练的前置条件。
...
| 奖励/折扣 | 传感器面积归一化、完成奖励、γ=0.999 | 固定物理单位、99%成功奖励5、γ=1、小温度与决策成本 |
