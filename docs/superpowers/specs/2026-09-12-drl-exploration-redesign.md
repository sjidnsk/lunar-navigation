...
2026-09-18 修正默认采用 `γ=0.995`，允许配置 `0<γ<1`（可对照 0.999）。这是抑制自循环价值偏差的验证初值；长通道与绕行的延迟收益可能衰减，须单独验证。真实终止不 bootstrap，预算截断仍 bootstrap。
...
| 奖励/折扣 | 传感器面积归一化、完成奖励、γ=0.999 | 固定物理单位、无完成奖金、γ=0.995、小温度与决策成本 |
