...
### 2026-08-04 轮式地形跟随速度断言修订

正式 v2 场景回归证明，生产轮式 timing 会把世界坐标中的地形高差纳入轨迹位置，并把
`linear.z` 生成为该高差的时间导数。外部验证器此前把任何非零 `linear.z` 直接判为
`WHEEL_SPEED_LIMIT`，这与“验证线速度和三维线加速度不超过能力资料”的冻结设计不一致；
它会错误拒绝沿真实月面起伏行驶且速度远低于上限的参考。

本修订只更正仓库外断言，不改变生产规划器、能力资料、USD、快照、v2 场景锁或期望
Action 三元组：

- 轮式线速度按世界坐标三维模
  `sqrt(vx²+vy²+vz²)` 与前进或倒车上限比较。
- 前进/倒车仍由 XY 平面速度在车体 yaw 前向轴上的投影符号判定；非零地形跟随
  `vz` 本身不是失败条件。
- 原有三维线加速度模、制动、横向加速度、偏航速度、偏航加速度以及
  `wx=wy=0` 断言保持不变，因此 Z 方向变化仍受速度和加速度上限约束。
- 单元测试必须用字面构造的起伏轨迹独立证明：上限内的非零 `vz` 通过，而仅因
  三维速度模超过上限的轨迹仍以 `WHEEL_SPEED_LIMIT` 失败。不得从正式 Action 输出
  复制期望速度，也不得用重跑掩盖断言错误。
