...
5. 每移动 4 m 刷新局部图并重新规划，保留足够前视距离，同时避免跑出已确认地图窗口。
...
Demo 在停止位置同周期发布局部地图与 odometry，并复用同一 header stamp。该 stamp 是
`map_token`，只用于关联消息，不进行年龄或超时新鲜度判断。等待 ACK 期间不生成带新 stamp 的
规划 odometry，而是重复交付同一个停止位姿快照，直到该 token 被确认或当前请求被抢占。
...
- Demo 每执行 4 m 就停止、刷新局部图并重规划；因此正常情况下始终保有约 8 m 以上的已规划
  前视余量。
...
3. 在滚动 Action 中实现停止、等待 ACK、规划、执行授权、4 m 刷新和最多 2 次恢复。
...
- 移动累计达到 4 m 后停止并等待下一版地图，ACK 前不继续推进。
- 两次可恢复重试后终止，终止失败不重试。
