...
局部 XY 分辨率为 `0.2 m`，航向离散为 `32` 个方向，单次航向变化为
`delta_yaw = pi / 16`。能力文件显式包含：
...
半径 `0.2 m`、航向变化 `pi/16` 的圆弧局部终点由固定公式生成：

```text
x = radius * sin(delta_yaw) = 0.039018064403226 m
y = radius * (1 - cos(delta_yaw)) = 0.003842943919354 m
```
