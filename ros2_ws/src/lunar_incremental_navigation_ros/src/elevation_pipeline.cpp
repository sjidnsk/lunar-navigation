...
ElevationPipeline::ElevationPipeline(
    ...
    : capability_(std::move(capability)),
      ...
      bundle_(std::make_shared<const lunar::incremental_navigation::SnapshotBundle>()) {}
...
lunar::incremental_navigation::ElevationUpdateResult ElevationPipeline::ApplyLocal(
  ...
  if (adapted.value) {
    return ApplyLocal(*adapted.value);
  }
  std::scoped_lock lock{work_mutex_};
...
bool ElevationPipeline::RunFineDerivation() {
  ...
  while (true) {
    auto replacement =
        std::make_shared<const lunar::incremental_navigation::SnapshotBundle>(
            lunar::incremental_navigation::SnapshotBundle{
                .fine = next, .guidance = current->guidance});
...
bool ElevationPipeline::RunGuidanceDerivation() {
  ...
  const auto captured =
      std::atomic_load_explicit(&bundle_, std::memory_order_acquire);
  const auto fine = captured->fine;
  if (!fine) {
    return false;
  }
  const auto previous = captured->guidance;
  {
    std::scoped_lock lock{work_mutex_};
  ...
  while (true) {
    auto replacement =
        std::make_shared<const lunar::incremental_navigation::SnapshotBundle>(
            lunar::incremental_navigation::SnapshotBundle{
                .fine = current->fine, .guidance = next});
    if (std::atomic_compare_exchange_weak_explicit(
            &bundle_, &current, std::move(replacement),
            std::memory_order_acq_rel, std::memory_order_acquire)) {
      break;
...
lunar::incremental_navigation::SnapshotBundle ElevationPipeline::CaptureBundle() const {
  return *std::atomic_load_explicit(&bundle_, std::memory_order_acquire);
}
