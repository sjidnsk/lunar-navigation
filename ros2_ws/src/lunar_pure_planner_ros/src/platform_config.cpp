...
using lunar::pure_planning::Interval;
using lunar::pure_planning::LeggedBodyPrimitive;
using lunar::pure_planning::LeggedCapability;
using lunar::pure_planning::LeggedPrimitiveKind;
using lunar::pure_planning::PlatformCapability;
using lunar::pure_planning::Pose3;
using lunar::pure_planning::Quaternion;
using lunar::pure_planning::Vec2;
using lunar::pure_planning::Vec3;
using lunar::pure_planning::WheeledCapability;
using lunar::pure_planning::WheelMotionPrimitive;
using lunar::pure_planning::WheelPrimitiveKind;
...
PlatformCapability ParseHopper(const YAML::Node& root) {
  ...
  RequireKeys(node, {"specific_impulse_s", "reference_total_mass_kg", "reference_propellant_mass_kg",
                     ...
                     "runtime_fallback_allowed", "landing_support_radius_m", "flight_collision_radius_m",
                     "maximum_landing_plane_residual_m", "landing_lateral_margin_m", "flight_map_margin_m",
  ...
  return HopperCapability{
      ...
      .reference_horizontal_range_m = Positive(node, "reference_horizontal_range_m"),
      .reference_elevation_delta_m = Finite(node, "reference_elevation_delta_m"),
      .runtime_fallback_allowed = Boolean(node, "runtime_fallback_allowed"),
      .landing_support_radius_m = Positive(node, "landing_support_radius_m"), .flight_collision_radius_m = Positive(node, "flight_collision_radius_m"),
      .maximum_landing_plane_residual_m = NonNegative(node, "maximum_landing_plane_residual_m"),
      .landing_lateral_margin_m = NonNegative(node, "landing_lateral_margin_m"), .flight_map_margin_m = NonNegative(node, "flight_map_margin_m"),
      .reachability_delta_v_margin_ratio = NonNegative(node, "reachability_delta_v_margin_ratio"),
      .standard_gravity_mps2 = Positive(node, "standard_gravity_mps2"), .maximum_landing_slope_rad = Positive(node, "maximum_landing_slope_rad"),
  };
}
...
PlatformConfigResult LoadPlatformConfig(const std::filesystem::path& path,
                                        const std::string_view platform) {
  if (platform != "wheel" && platform != "legged" && platform != "hopper") {
