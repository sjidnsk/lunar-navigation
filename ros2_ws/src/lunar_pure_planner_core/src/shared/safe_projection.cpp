...
GridSearchProjectionView SafeProjection::GridSearchView() const & noexcept {
  return GridSearchProjectionView{
      .map = source_map_.get(),
      .hard_feasible = hard_feasible_mask_,
      .clearance_m = clearance_m_,
      .slope_rad = slope_rad_,
      .roughness_m = roughness_m_,
      .traversal_cost = traversal_cost_,
  };
}
