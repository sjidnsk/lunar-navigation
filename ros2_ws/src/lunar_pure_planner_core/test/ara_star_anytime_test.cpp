...
TEST(AraStarAnytime, TimesOutBeforeFindingAnIncumbent) {
  ...
  problem.expand = [](std::size_t, std::vector<GraphEdge>&) {};
...
TEST(AraStarAnytime, TimesOutDuringChunkedStateInitializationBeforeHeuristic) {
  std::size_t now_calls = 0U;
  std::size_t heuristic_calls = 0U;
  std::size_t expand_calls = 0U;
  AraStarProblem problem;
  problem.state_count = 65536U;
  problem.start_state = 0U;
  problem.heuristic = [&](std::size_t) {
    ++heuristic_calls;
    return 0.0;
  };
  ...
  problem.control.deadline = SteadyClock::time_point{4ms};
  problem.control.now = [&] {
    return SteadyClock::time_point{std::chrono::milliseconds{now_calls++}};
  };
  ...
  EXPECT_EQ(result.status, AraStarStatus::kTimedOut);
  EXPECT_TRUE(result.deadline_reached);
  EXPECT_EQ(heuristic_calls, 0U);
  EXPECT_EQ(expand_calls, 0U);
}

TEST(AraStarAnytime, CancelsDuringChunkedStateInitializationBeforeHeuristic) {
  ...
  problem.control.stop_token = stop.get_token();
  problem.control.deadline = SteadyClock::time_point::max();
  problem.control.now = [&] {
    if (now_calls++ == 3U) {
      stop.request_stop();
    }
...
TEST(AraStarAnytime, ReturnsIncumbentWhenDeadlineArrivesDuringImprovement) {
  ...
  problem.heuristic = [](std::size_t) { return 0.0; };
  problem.is_goal = [](std::size_t state) { return state == 1U; };
  problem.expand = [](const std::size_t state, std::vector<GraphEdge>& edges) {
    if (state == 0U) {
      edges = {{.target_state = 1U, .cost = 1.0, .stable_index = 7U}};
    }
  };
  // Five state vectors each add four controlled initialization reads for this
  // two-state fixture before the original improvement sequence begins.
  problem.control.deadline = SteadyClock::time_point{24ms};
  problem.control.now = [&tick] {
    return SteadyClock::time_point{std::chrono::milliseconds{tick++}};
  };

  const auto result = SearchAnytimeAraStar(problem);

  ASSERT_EQ(result.status, AraStarStatus::kSolved)
      << result.reason_code << ":clock_reads=" << tick;
  EXPECT_TRUE(result.deadline_reached);
...
TEST(AraStarAnytime, GivesCancellationPriorityAtReachableRekeyDeadline) {
  std::stop_source stop_source;
  std::vector<std::size_t> expand_calls(4U);
  std::size_t now_calls = 0U;
  ...
  problem.control.now = [&] {
    ...
    return SteadyClock::time_point{-1ms};
  };
...
TEST(AraStarAnytime, RejectsInvalidEdgesAndHeuristicsWhenUsed) {
  ...
  invalid_edge.expand = [](std::size_t, std::vector<GraphEdge>& edges) {
  ...
  invalid_heuristic.expand = [](std::size_t, std::vector<GraphEdge>& edges) {
...
TEST(AraStarAnytime, RejectsNonFiniteEdgesBeforeOrderingThem) {
  for (const double invalid_cost :
    ...
    problem.heuristic = [](std::size_t) { return 0.0; };
    problem.is_goal = [](std::size_t state) { return state == 1U; };
    problem.expand = [invalid_cost](std::size_t,
                                    std::vector<GraphEdge>& edges) {
      edges = {
...
TEST(AraStarAnytime, ChecksCancellationAndDeadlineAfterTheLastExpansion) {
  ...
  canceled.expand = [&stop_source](std::size_t, std::vector<GraphEdge>&) {
    stop_source.request_stop();
  };
  ...
  timed_out.expand = [&expired](std::size_t, std::vector<GraphEdge>&) {
    expired = true;
  };
...
TEST(AraStarAnytime, PreservesIncumbentAndMetricsWhenSearchAllocationsFail) {
  for (const bool throw_bad_alloc : {false, true}) {
    ...
    problem.heuristic = [](std::size_t) { return 0.0; };
    problem.is_goal = [](std::size_t state) { return state == 1U; };
    problem.expand = [throw_bad_alloc](const std::size_t state,
                                       std::vector<GraphEdge>& edges) {
      if (state == 0U) {
        edges = {
...
TEST(AraStarAnytime, ReconstructsAPathWithTheLargestStableEdgeIndex) {
  ...
  problem.heuristic = [](std::size_t) { return 0.0; };
  problem.is_goal = [](std::size_t state) { return state == 1U; };
  problem.expand = [](std::size_t, std::vector<GraphEdge>& edges) {
...
TEST(AraStarAnytime, UsesGuidanceOnlyToBreakEqualPriorityTies) {
  const auto make_problem = [] {
    ...
    problem.heuristic = [](std::size_t) { return 0.0; };
    problem.is_goal = [](const std::size_t state) { return state == 3U; };
    problem.expand = [](const std::size_t state,
                        std::vector<GraphEdge>& edges) {
      if (state == 0U) {
        edges = {
            {.target_state = 1U, .cost = 1.0, .stable_index = 1U},
...
TEST(AraStarAnytime, PrioritizesGuidedFirstSolutionBeforeLowerCostBranch) {
  AraStarProblem problem;
  problem.state_count = 4U;
  problem.start_state = 0U;
  problem.heuristic = [](std::size_t) { return 0.0; };
