...
class PlatformConfig:
    ...
    def actor_context(self, pose: Pose, *, linear_speed_mps: float,
                      angular_speed_radps: float, sensor_range_m: float,
                      sensor_fov_rad: float) -> np.ndarray:
        return np.asarray([
            np.cos(pose.yaw), np.sin(pose.yaw), linear_speed_mps / 0.2,
            angular_speed_radps / self.maximum_spin_rate_radps,
            sensor_range_m / 10.0, sensor_fov_rad / np.pi,
            self.footprint_radius_m, self.maximum_forward_speed_mps / 0.2,
        ], dtype=np.float32)
