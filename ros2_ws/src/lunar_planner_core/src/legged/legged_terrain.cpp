...
[[nodiscard]] LeggedSweepResult
SweepFailure(std::string reason_code, const std::size_t sample_count = 0U) {
...
[[nodiscard]] bool
ValidCapability(const LeggedCapability &capability) noexcept {
...
EvaluateLeggedTerrainCell(const shared::SafeProjection &projection,
  ...
  if (!std::isfinite(result.slope_rad) ||
      result.slope_rad > capability.maximum_slope_rad + kComparisonTolerance ||
      result.slope_rad >
          projection.maximum_slope_rad() + kComparisonTolerance) {
    AddReason(result, "LEGGED_SLOPE_LIMIT");
  }
  if (!std::isfinite(result.roughness_m) || result.roughness_m < 0.0 ||
      result.roughness_m >
          capability.maximum_roughness_m + kComparisonTolerance) {
  ...
  if (!projection.IntrinsicFeasible(cell) && result.rejection_reasons.empty()) {
...
LeggedSweepResult
ValidateLeggedBodySweep(const LeggedPose &source, const LeggedPose &target,
  ...
  if (!IsFinite(source) || !IsFinite(target) ||
      ...
      projection.source_map() == nullptr || !ValidCapability(capability)) {
    return SweepFailure("LEGGED_SWEEP_REQUEST_INVALID");
  }
  const double support_radius = std::hypot(capability.body_half_extent_m.x,
  ...
  const double maximum_step = projection.source_map()->resolution_m() * 0.25;
  ...
  for (std::size_t sample = 0U; sample <= subdivisions; ++sample) {
    ...
    const double extent_x = std::abs(cosine) * capability.body_half_extent_m.x +
                            std::abs(sine) * capability.body_half_extent_m.y;
    const double extent_y = std::abs(sine) * capability.body_half_extent_m.x +
    ...
    const auto center_cell =
        map.PositionToCell(Vec2{.x = center_x, .y = center_y});
