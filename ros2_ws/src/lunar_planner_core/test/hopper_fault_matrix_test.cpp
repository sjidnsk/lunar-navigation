...
TEST(HopperFaultMatrix, RejectsLandingSlopeRoughnessAndPlaneResidual) {
  ...
  SetFloat(
      roughness.world.local_map, "elevation_variance", 8U, 6U, 0.0225F);
...
TEST(HopperFaultMatrix, RejectsInsufficientLandingAreaAndClearance) {
  ...
  std::get<HopperCapability>(area.capability)
      .minimum_landing_region_area_m2 = 0.3;
  ExpectNoNewHop(planner.Plan(area));

  auto lateral = test::MakeValidHopperInput();
  MakeExactCellGoal(lateral);
  std::get<HopperCapability>(lateral.capability)
      .minimum_lateral_clearance_m = 0.3;
  ...
  std::get<HopperCapability>(overhead.capability)
      .minimum_overhead_clearance_m = 0.2;
...
TEST(HopperFaultMatrix, RejectsLaunchImpulseFlightAndLandingLimits) {
  ...
  std::get<HopperCapability>(launch_speed.capability)
      .maximum_launch_speed_mps = 0.5;
...
TEST(HopperFaultMatrix, RejectsAttitudeUnknownAndNumericalUncertainty) {
  ...
  SetFloat(
      uncertainty.world.local_map, "obstacle_variance", 8U, 6U, 0.05F);
